...
@section('title', 'Edit Pemilik - Admin RSHP')

@section('content')
    <div style="background:#fff;padding:2rem;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.06)">
        <div style="margin-bottom:1.5rem">
            <h2 style="color:#2463d6;font-size:1.75rem;margin-bottom:0.5rem">Edit Pemilik</h2>
        ...
        <form action="{{ route('admin.pemilik.update', $pemilik->idpemilik) }}" method="POST">
            ...
            <div style="margin-bottom:1.5rem">
                ...
                <textarea name="alamat" required rows="3" style="width:100%;padding:0.75rem;border:1px solid #d1d5db;border-radius:6px;font-size:1rem;font-family:inherit">{{ old('alamat', $pemilik->alamat) }}</textarea>
            ...
            <div style="margin-bottom:1.5rem">
                ...
                <input type="text" name="no_wa" value="{{ old('no_wa', $pemilik->no_wa) }}" required style="width:100%;padding:0.75rem;border:1px solid #d1d5db;border-radius:6px;font-size:1rem" placeholder="Contoh: 081234567890">
