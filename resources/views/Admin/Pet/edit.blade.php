        ...
        <div class="row">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <form action="{{ route('admin.pet.update', $pet->id_pet) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            @if(session('error'))
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('nama_pet') is-invalid @enderror" 
                                       id="nama_pet" name="nama_pet" value="{{ old('nama_pet', $pet->nama_pet) }}" required>
                                @error('nama_pet')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('tanggal_lahir') is-invalid @enderror" 
                                       id="tanggal_lahir" name="tanggal_lahir" value="{{ old('tanggal_lahir', $pet->tanggal_lahir) }}" required>
                                @error('tanggal_lahir')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <select class="form-select @error('idras_hewan') is-invalid @enderror" 
                                    ...
                                    @foreach($rasHewan as $ras)
                                        <option value="{{ $ras->idras_hewan }}" 
                                            ...
                                            {{ $ras->nama_jenis_hewan }} - {{ $ras->nama_ras }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('idras_hewan')
