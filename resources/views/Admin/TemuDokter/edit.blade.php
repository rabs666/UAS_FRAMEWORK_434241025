        ...
        <div class="row">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <form action="{{ route('admin.temu_dokter.update', $temuDokter->id_temu_dokter) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            @if(session('error'))
                                ...
                                <select class="form-select @error('id_pet') is-invalid @enderror" 
                                    ...
                                    @foreach($pets as $pet)
                                        <option value="{{ $pet->id_pet }}" 
                                            {{ old('id_pet', $temuDokter->id_pet) == $pet->id_pet ? 'selected' : '' }}>
                                            {{ $pet->nama_pet }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-select @error('id_dokter') is-invalid @enderror" 
                                    ...
                                    @foreach($dokters as $dokter)
                                        <option value="{{ $dokter->id_dokter }}" 
                                            {{ old('id_dokter', $temuDokter->id_dokter) == $dokter->id_dokter ? 'selected' : '' }}>
                                            {{ $dokter->name }}
                                        </option>
                                    @endforeach
                                </select>
