...
@section('title', 'Edit Detail Rekam Medis - Dokter')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <form action="{{ route('dokter.detail_rekam_medis.update', $detailRekam->iddetail_rekaman_medis) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        @if(session('error'))
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('idrekam_medis') is-invalid @enderror" id="idrekam_medis" name="idrekam_medis" required>
                                ...
                                @foreach($rekamMedis as $rm)
                                    <option value="{{ $rm->idrekam_medis }}" {{ (old('idrekam_medis', $detailRekam->idrekam_medis) == $rm->idrekam_medis) ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('idkode_tindakan_terapi') is-invalid @enderror" id="idkode_tindakan_terapi" name="idkode_tindakan_terapi" required>
                                ...
                                @foreach($kodeTindakan as $kt)
                                    <option value="{{ $kt->idkode_tindakan_terapi }}" {{ (old('idkode_tindakan_terapi', $detailRekam->idkode_tindakan_terapi) == $kt->idkode_tindakan_terapi) ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('hasil') is-invalid @enderror" id="hasil" name="hasil" rows="3">{{ old('hasil', $detailRekam->hasil) }}</textarea>
                            @error('hasil')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('catatan') is-invalid @enderror" id="catatan" name="catatan" rows="3">{{ old('catatan', $detailRekam->catatan) }}</textarea>
                            @error('catatan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
