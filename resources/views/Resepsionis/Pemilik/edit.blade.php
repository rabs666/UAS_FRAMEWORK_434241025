...
@section('title', 'Edit Pemilik - Resepsionis')
            ...
            <form action="{{ route('resepsionis.pemilik.update', $pemilik->idpemilik) }}" method="POST">
              ...
              <div class="card-body">
                ...
                <div class="alert alert-info">
                  <i class="bi bi-info-circle"></i> <strong>Nama Pemilik:</strong> {{ $pemilik->nama_pemilik ?? 'Tidak tersedia' }}
                </div>

                <div class="mb-3">
                  <label for="alamat" class="form-label">Alamat</label>
                  <textarea class="form-control @error('alamat') is-invalid @enderror" id="alamat" name="alamat" rows="3">{{ old('alamat', $pemilik->alamat) }}</textarea>
                  @error('alamat')
                ...
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control @error('no_wa') is-invalid @enderror" id="no_wa" name="no_wa" value="{{ old('no_wa', $pemilik->no_wa) }}" required>
                  @error('no_wa')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>
